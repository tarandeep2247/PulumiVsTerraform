...
# Set Pulumi CLI argument for auto-approval
export PULUMI_SKIP_CONFIRMATIONS=true

# Get Pulumi command as user input
read -p "Enter the Pulumi command: " PULUMI_COMMAND

# Output file for resource utilization results
OUTPUT_FILE="pulumi_utilization.log"

# Create or truncate the output file
echo "Timestamp,CPU Usage (%),Memory Usage (MB),Disk I/O (KB/s)" > "$OUTPUT_FILE"

# Run Pulumi and get the PID
(eval "$PULUMI_COMMAND") &
PID=$!
...
# Display utilization one last time after Pulumi finishes
display_utilization
